...
def populate_weather_status(sender, **kwargs):
    if sender.name != "apps.weather": return
    data = [
        ...
        {"name": "Thunderstorm", "image_path": "images/weather_status/thunderstorm.png"},
        {"name": "Haze", "image_path": "images/weather_status/haze.png"},
    ]
    for item in data:
        WeatherStatus.objects.update_or_create(name=item["name"], defaults={"image_path": item["image_path"]})